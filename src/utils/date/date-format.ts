...
const pad = (value: string | number, len = 2): string => {
	return value.toString().length < len ? pad("0" + value, len) : value.toString();
};
...
export const format = (inputDate: string | Date, pattern = "yyyy-mm-dd", utc = false) => {
	const date = typeof inputDate === "string" ? new Date(inputDate) : inputDate;
	if (isNaN(date.getTime())) return "";

	const mask = String(masks[pattern] || pattern);
