		...
		<>
			<ControlGroup title={"XAxis"}>
				<Control name={"title"} type={"ReactNode"}>
					<HTMLControl html={state.title?.toString() ?? ""} onChange={(html) => onChange({ title: html })} />
				</Control>
				<Control name={"description"} type={"ReactNode"}>
					<HTMLControl html={state.description?.toString() ?? ""} onChange={(html) => onChange({ description: html })} />
				</Control>
			</ControlGroup>
		</>
	);
