...
describe("src/components/LinearGradient", () => {
	...
	it("Should support opacity:0.5 in linear-gradient strings", () => {
		...
		console.log(GradientUtils.parse("linear-gradient(rgb(0, 0,0) opacity:0.5, rgb(155, 25, 35)  opacity:0.8)"));
		const gradient = getByTestId("a");
		expect(gradient).toBeInTheDocument();
		const stops = gradient.querySelectorAll("stop");
		expect(stops).toHaveLength(2);
	...
	it("Should support mask:linear-gradient(...) with VALUE stops not coordinate stops.", () => {
		...
		expect(stops[0].getAttribute("stop-opacity")).toBe(null);
		expect(stops[0].getAttribute("offset")).toBe(MathUtils.scale(xForValue(400), MOCK_CONTEXT.viewbox.x, 100).toString() + "%");
		expect(stops[1].getAttribute("stop-color")).toBe("rgb(0, 0, 255)");
		expect(stops[1].getAttribute("stop-opacity")).toBe(null);
		expect(stops[1].getAttribute("offset")).toBe(MathUtils.scale(xForValue(400.1), MOCK_CONTEXT.viewbox.x, 100).toString() + "%");
		expect(stops[2].getAttribute("stop-color")).toBe("rgb(0, 0, 255)");
		expect(stops[2].getAttribute("stop-opacity")).toBe(null);
		expect(stops[2].getAttribute("offset")).toBe(MathUtils.scale(xForValue(1000), MOCK_CONTEXT.viewbox.x, 100).toString() + "%");
	});
