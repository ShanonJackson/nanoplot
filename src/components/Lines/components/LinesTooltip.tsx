...
import { CartesianDataset, GraphContext, useDatasets, useGraph, useIsZooming } from "../../../hooks/use-graph/use-graph";
...
import { xCoordinateFor, yCoordinateFor } from "../../../utils/coordinates/coordinates";
...
const LinesTooltipComponent = ({ tooltip, joints = true, datasets, zoneRef: ref, ...rest }: Props) => {
	...
	const xForValue = xCoordinateFor({ domain, viewbox });
	const yForValue = yCoordinateFor({ domain, viewbox });
	...
	const pointsFor = (dset: Pick<GraphContext, "domain" | "colors" | "data">) => {
		...
		if (!GraphUtils.isXYData(dset.data)) return undefined;
		return dset.data
			.flatMap((line, i) => {
				...
				const stroke = (() => {
					...
					return line.stroke ?? dset.colors[i] ?? dset.colors.at(-1);
				})();
	...
	const points = Object.values(dsets)
		.flatMap((dset) => pointsFor(dset))
		.concat(pointsFor({ data, domain, colors }))
