        ...
        <div className="flex flex-col h-[400px] w-[768px] bg-white">
            <div className="flex justify-between m-2 p-1">
                <div className="flex justify-start font-bold">
                    ...
                    <p className="text-gray-400">by <span className="border-b-2 border-dotted border-gray-400">Country</span></p>
                </div>
            ...
            <div className="flex">
                ...
                <div className="flex flex-col flex-1 w-full text-gray-400 text-sm font-bold">
                    ...
                    <div>
                        {DATA.sort((a, b) => b.value - a.value).slice(0, 7).map((item, index) => {
                            return <div key={item.countryName} className="flex flex-col px-4 pt-2">
                                <div className="flex justify-between ">
                                    <span>{item.countryName}</span><span>{item.value>1000?((item.value/1000).toFixed(item.value % 1000 === 0 ? 0 : 1) + "K"): item.value}</span>
                                </div>
                                <div className="w-full h-0.5 bg-gray-200 rounded relative overflow-visible mt-2">
                                    <div className="h-1 -top-0.5 absolute bg-blue-500 rounded" style={{ width: `${Math.floor(item.value*100/50000)}%` }}></div>
                                </div>
                            </div>
                        })}
                    </div>
