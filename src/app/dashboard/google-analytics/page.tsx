				...
				<nav className="h-16 bg-white flex items-center justify-between text-gray-500 font-bold">
					<div className="flex items-center [&>*]:mx-4 [&>*]:px-4">
						...
						<div className="relative w-[512px] h-10 bg-gray-100 rounded !p-0">
							...
							<input className="w-[512px] h-10 bg-gray-100 rounded pl-12" placeholder='Try searching "measurement ID"' />
						</div>
					</div>
					<div className="flex items-center mx-2 [&>*]:mx-2 [&>*]:cursor-pointer">
						...
						<div className="flex items-center justify-center w-6 h-6 rounded-full">
							<img src="/assets/icons/icn_help.png" />
						</div>
				...
				<aside className="bg-gray-100 h-full p-6 shadow-md w-16 border-r border-gray-300">
					<div className="flex flex-col h-full justify-between items-center">
						<ul className="space-y-4 text-gray-700 font-medium [&>*]:cursor-pointer">
							{[
								"/assets/icons/icn_home.png",
								"/assets/icons/icn_reports.png",
								"/assets/icons/icn_check-trend.png",
								"/assets/icons/icn_target.png",
							].map((item, index) => {
								return (
									<li
										key={index}
										className={`flex justify-center items-center w-10 h-10 rounded-full ${index === 1 ? "bg-blue-400" : "hover:bg-gray-200"}`}
									>
										<img className="w-8 h-8" src={item} />
									</li>
								);
							})}
						</ul>
				...
				<main className="flex flex-col flex-1 overflow-auto mx-8 mt-6 bg-gray-100">
					...
					<div className="flex items-center justify-between mt-4 h-10 text-gray-500 font-bold">
						...
						<div className="flex justify-end [&>*]:cursor-pointer">
							{[
								"/assets/icons/icn_notes.png",
								"/assets/icons/icn_split.png",
								"/assets/icons/icn_share.png",
								"/assets/icons/icn_trend.png",
							].map((item, index) => {
								return (
									<div key={index} className="mx-2 w-8 h-8 hover:bg-gray-200 rounded">
										<img src={item} />
									</div>
								);
							})}
